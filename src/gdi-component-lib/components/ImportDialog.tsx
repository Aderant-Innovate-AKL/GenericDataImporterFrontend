...
export default function ImportDialog({
  ...
  const [progress, setProgress] = useState<OperationProgress | undefined>(undefined);
  ...
  const start = async () => {
    ...
    try {
      const res = await service.extractWithPolling(file, context, (p) => setProgress(p));
      setResult(res);
      onSuccess?.(res);
    } catch (err: unknown) {
  ...
  return (
    <Dialog open={open} onClose={onClose} fullWidth maxWidth="lg">
      ...
      <DialogContent sx={{ position: 'relative' }}>
        {progress && !result && (
          <LoadingOverlay
            message={progress.label ?? 'Importing...'}
            progress={progress}
          />
        )}
